...
int main(){
	...
	double fv = bondPricing::FutureValue(P, nPeriods, r);

	cout.setf(ios_base::fixed, ios_base::floatfield);
	cout << "Future Value: " << fv << endl;

	long frequency = 2;
	double fv2 = bondPricing::FutureValue(P, nPeriods, r, frequency);
//	cout.setf(0, ios_base::floatfield);
	...
	dblVector futureValues(5); // For five years
	for (long j = 0; j < 4; j++)
	{ // The first 4 years
	...
	nPeriods = 5; // Redundant in a sense since this is in the dblVector
	r = 0.0625;
